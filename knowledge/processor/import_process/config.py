...
# 显式指定 .env 路径（位于 knowledge/.env），避免从不同工作目录运行时找不到
...
class ImportConfig:
    ...
    bge_m3_path: str = field(
        default_factory=lambda: os.getenv("BGE_M3_PATH", "BAAI/bge-m3")
    )
    bge_device: str = field(
        default_factory=lambda: os.getenv("BGE_DEVICE", "cuda:0")
    )
    bge_fp16: bool = field(
        default_factory=lambda: os.getenv("BGE_FP16", "True").lower() == "true"
    )
